...
async def test_egedal_user_refresh(
    ...
    # omada_mock([bad_user])
    # await assert queue == 1
    #
    # omada_mock([fixed_user])
    # await assert queue == 2
    #
    # run integration
    # await assert user created
    # await assert queue == 0


    # Ensure all queues are empty
    queues = await rabbitmq_management_client.get("queues")
    print(queues.json())
    for queue in queues.json():
        print(queue)
        messages = queue["messages_ready"] + queue["messages_unacknowledged"]
        assert messages == 0, queue["name"]
