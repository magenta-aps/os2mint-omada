...
async def test_silkeborg_sd_user(
    ...
    omada_manually_created_uuid = engagement_types["omada_manually_created"]  # Omada
    deltid_uuid = engagement_types["delted"]  # SD

    engagement_job_functions = await mo.get_classes("engagement_job_function")
    medarbejder_uuid = engagement_job_functions["Medarbejder"]

    sd_engagement = await graphql_client.create_engagement(
        input=EngagementCreateInput(
            user_key="DK-1337",
            person=employee.uuid,
            org_unit=org_unit,
            engagement_type=deltid_uuid,
            job_function=medarbejder_uuid,
            validity=RAValidityInput(
                from_=datetime(2000, 1, 1, tzinfo=timezone(timedelta(hours=1))),
                to=datetime(2020, 1, 1, tzinfo=timezone(timedelta(hours=1))),
            ),
        )
    )
    ...
    # TODO: omada engagement should be deleted (but it probably isn't because the event-handler bails when user is non-manual).
    # TODO: addresses should be associated with the sd engagement
    # TODO: it-users should be associated with the sd engagement
