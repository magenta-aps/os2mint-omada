# SPDX-FileCopyrightText: Magenta ApS <https://magenta.dk>
# SPDX-License-Identifier: MPL-2.0

from fastapi import FastAPI
from fastramqpi.events import GraphQLEvents
from fastramqpi.events import Namespace
from fastramqpi.main import FastRAMQPI

from os2mint_omada import api
from os2mint_omada.autogenerated_graphql_client import GraphQLClient
from os2mint_omada.config import Settings
from os2mint_omada.omada.api import OmadaAPI
from os2mint_omada.omada.api import create_client
from os2mint_omada.omada.event_generator import OMADA_NAMESPACE
from os2mint_omada.omada.event_generator import OmadaEventGenerator
from os2mint_omada.sync.frederikshavn.events import (
    mo_listeners as frederikshavn_mo_listeners,
)
from os2mint_omada.sync.frederikshavn.events import mo_router as frederikshavn_mo_router
from os2mint_omada.sync.frederikshavn.events import (
    omada_listeners as frederikshavn_omada_listeners,
)
from os2mint_omada.sync.frederikshavn.events import (
    omada_router as frederikshavn_omada_router,
)
from os2mint_omada.sync.frederikshavn.events import (
    subject_builder as frederikshavn_subject_builder,
)
from os2mint_omada.sync.silkeborg.events import mo_listeners as silkeborg_mo_listeners
from os2mint_omada.sync.silkeborg.events import mo_router as silkeborg_mo_router
from os2mint_omada.sync.silkeborg.events import (
    omada_listeners as silkeborg_omada_listeners,
)
from os2mint_omada.sync.silkeborg.events import omada_router as silkeborg_omada_router
from os2mint_omada.sync.silkeborg.events import (
    subject_builder as silkeborg_subject_builder,
)


def create_app() -> FastAPI:
    """FastRAMQPI application factory.

    Args:
        **kwargs: Additional keyword arguments passed to the settings module.

    Returns: FastAPI application.
    """
    settings = Settings()

    match settings.customer:
        case "frederikshavn":
            mo_router = frederikshavn_mo_router
            mo_listeners = frederikshavn_mo_listeners
            omada_router = frederikshavn_omada_router
            omada_listeners = frederikshavn_omada_listeners
            subject_builder = frederikshavn_subject_builder
        case "silkeborg":
            mo_router = silkeborg_mo_router
            mo_listeners = silkeborg_mo_listeners
            omada_router = silkeborg_omada_router
            omada_listeners = silkeborg_omada_listeners
            subject_builder = silkeborg_subject_builder
        case _:
            raise ValueError("Improperly configured")

    fastramqpi = FastRAMQPI(
        application_name="omada",
        settings=settings.fastramqpi,
        graphql_version=30,
        graphql_client_cls=GraphQLClient,
        graphql_events=GraphQLEvents(
            declare_namespaces=[Namespace(name=OMADA_NAMESPACE)],
            declare_listeners=mo_listeners + omada_listeners,
        ),
    )
    fastramqpi.add_context(settings=settings)
    context = fastramqpi.get_context()

    # FastAPI router
    app = fastramqpi.get_app()
    app.include_router(api.router)

    # MO and Omada GraphQL event handlers
    app.include_router(mo_router)
    app.include_router(omada_router)

    # Omada API
    omada_client = create_client(settings.omada)
    # priority ensures the client is started before the event handlers and generator
    # tries to use it.
    fastramqpi.add_lifespan_manager(omada_client, priority=600)
    omada_api = OmadaAPI(url=settings.omada.url, client=omada_client)
    fastramqpi.add_context(omada_api=omada_api)
    fastramqpi.add_context(subject_builder=subject_builder)

    # Omada event generator
    omada_event_generator = OmadaEventGenerator(
        settings=settings.omada,
        api=omada_api,
        context=context,
        subject_builder=subject_builder,
    )
    fastramqpi.add_lifespan_manager(omada_event_generator, priority=1101)

    return app
