...
# The namespace and routing key used for the integration's own Omada events in
# MO's event system. All Omada user changes (create/update/delete) share a single
# routing key; the handlers always reconcile based on the latest state, so the
# change type does not need to be encoded in the event.
OMADA_NAMESPACE = "omada"
OMADA_ROUTING_KEY = "user"


class OmadaEvent(StrEnum):
    """Type of change detected for an Omada user (used for logging)."""
...
class OmadaEventGenerator(AsyncContextManager):
    def __init__(
        ...
        api: OmadaAPI,
        context: Context,
        subject_builder: Callable[[OmadaUser], OmadaEventSubject],
    ) -> None:
        ...
        which are sent to MO's event system.

        Args:
            settings: Omada-specific settings.
            api: OmadaAPI instance.
            context: FastRAMQPI context, used to obtain the MO GraphQL client at
                runtime (it is only added to the context once the application has
                started).
            subject_builder: Customer-specific function building the event subject
                (the identifiers) for an Omada user.
        """
    ...
    async def generate(self) -> None:
        ...
        for uid in old_users.keys() | new_users.keys():
            ...
            if old is None:
                ...
                payload = new
            # Send to MO's event system. Only the identifiers (see OmadaEventSubject) are
            # sent as the subject, since MO does not allow sending whole objects.
            logger.info("Detected Omada event", change=event, uid=uid)
            assert payload is not None  # mypy is so dumb
            graphql_client = self.context["graphql_client"]
            await graphql_client.send_event(
                input=EventSendInput(
                    namespace=OMADA_NAMESPACE,
                    routing_key=OMADA_ROUTING_KEY,
                    subject=self.subject_builder(payload).json(),
                )
            )
