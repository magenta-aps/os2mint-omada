...
from os2mint_omada.autogenerated_graphql_client import (
    GetEmployeeUuidFromEngagementRegistrationsObjectsEngagementRegistration as _EngagementRegistration,
)
from os2mint_omada.autogenerated_graphql_client import (
    GetEmployeeUuidFromItuserRegistrationsObjectsITUserRegistration as _ITUserRegistration,
)
...
class MO:
    ...
    async def get_employee_uuid_from_engagement(self, eng_uuid: UUID) -> UUID | None:
        # Resolved via registrations so it also works for deleted engagements.
        registrations = await self.graphql_client.get_employee_uuid_from_engagement(
            uuids=[eng_uuid]
        )
        uuids = {
            validity.person_response.uuid
            for registration in registrations.objects
            if isinstance(registration, _EngagementRegistration)
            for validity in registration.validities
        }
        # None if the engagement never existed; an error if different UUIDs.
        return only(uuids)

    async def get_employee_uuid_from_ituser(self, ituser_uuid: UUID) -> UUID | None:
        # Resolved via registrations so it also works for deleted IT-users.
        registrations = await self.graphql_client.get_employee_uuid_from_ituser(
            uuids=[ituser_uuid]
        )
        uuids = {
            validity.person_response.uuid
            for registration in registrations.objects
            if isinstance(registration, _ITUserRegistration)
            for validity in registration.validities
            if validity.person_response is not None
        }
        # None if the IT-user never existed; an error if different UUIDs.
        return only(uuids)
